...
c          print *, 'bsize2=',bsize2
     ...
     +            recl=bsize*(6*nst+3)/8)   
        ...
        if(nve==1) then
c          open(27,file=invel,access='direct',form='unformatted',
c     +         recl=bsize*nxt)
c          print *, 'bsize=',bsize
        else
c          open(27,file=invel,access='direct',form='unformatted',recl=bsize2*nx*nyt)
c          open(27,file=invel,access='direct',form='unformatted',recl=bsize2*nxt)
        end if
...
C     CLOSE ALL FILES
     ...
     +           media,vp,vs,rho,io_opt,iperf,idyna,nvar,iost,partdeg,SoCalQ)
...
C     VP         P-WAVE VELOCITY, IF MEDIA=0       (REAL)   (RETURNED)
C     VS         S-WAVE VELOCITY, IF MEDIA=0       (REAL)   (RETURNED)
C     RHO        DENSITY, IF MEDIA=0               (REAL)   (RETURNED)
...
C     SGSN OUTPUT FILE OF ALL OTHER 19 VARIABLES   (CHAR)   (RETURNED)
      ...
      real :: mu_ss, mu_dd,dt,tmax, vp, vs, rho
      character (len=filen) :: chkp,chkj,insrc,invel
...
c     +           sxgro2,sygro2,szgro2,filen
     ...
     +  nbgz,nedz,nskpz,vpe,vse,den,nve,fac,Q0,ex,fp)
...
CCC   WRITE INPUT AND COMPUTED SIMULATION PARAMETERS TO CHECK FILE
  ...
  113 format('Q, FAC,Q0,EX,FP:',3G14.5)
