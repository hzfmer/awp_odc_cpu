...
CCC   ALLOCATE TEMPORARY SOURCE AND RECEIVER MEMORY
      select case (ifault)
        ...
        case(5)
            call initialize_ifault_5(coords,maxdim,dims,comm,rank,master,nxt,nyt,nzt,nsrc,nst,npsrc,nz,insrc,read_step,srcproc)
...
C     in the case of ifault=2, basically skip taxx etc, not needed
     ...
     +                 tstriny(i),tstrisx(i),tdmax(i),tstrisz(i),tmu_s(i),tmu_d(i)
        ...
        else if (ifault==1) then ! SMALL BINARY FILE
          allocate(tmpxx(nst))
          allocate(tmpyy(nst))
          allocate(tmpzz(nst))
          allocate(tmpxz(nst))
...
C ==
      ...
      if ((ifault==2).and.(npsrc.ne.0))  then
        ...
        open( 19, file=faultfile, form='unformatted', access='direct',recl=bsize*npsrc*nst*3, status='old') 
      ...
      subroutine initialize_ifault_5(coords,maxdim,dims,comm,rank,master,nxt,nyt,nzt,nsrc,nst,npsrc,nz,insrc,read_step,srcproc)
      use parstat
      implicit none
      integer, intent(in)                          :: maxdim, comm, rank, master, nxt, nyt, nzt
      integer, intent(in)                          :: nsrc, nst, nz, read_step
      integer, intent(inout)                       :: npsrc 
      integer, dimension(maxdim), intent(in)       :: coords,dims 
      character(len=180), intent(in)               :: insrc
      integer                                      :: ierr
      integer                                      :: num_blocks, blocklength
      integer(kind=MPI_OFFSET_KIND)                :: disp, stride
      integer                                      :: fh
      integer                                      :: type1, max_srcs 
      integer                                      :: nxp,nyp,nzp 
      integer                                      :: i,j,k
      integer                                      :: srcproc
      ...
      allocate(locs(nsrc*maxdim))
      allocate(src_locs(nsrc))

      ! read in source locations at all MPI ranks put in some function
      call MPI_BCast(insrc, 180, MPI_CHARACTER, 0, comm, ierr)
      call MPI_Type_vector(nsrc, maxdim, 3+6*nst, MPI_INT, type1, ierr)
      call MPI_Type_commit(type1, ierr)
      
      ! io stuff
      disp=0
      call MPI_File_Open(comm, insrc, MPI_MODE_RDONLY, MPI_INFO_NULL, fh, ierr) 
      call MPI_File_set_view(fh, disp, MPI_INT, type1, "native", MPI_INFO_NULL, ierr)
      call MPI_File_read_all(fh, locs, maxdim*nsrc, MPI_INT, MPI_STATUS_IGNORE, ierr)
      call MPI_File_close(fh,ierr)

      ! determine if rank has to read in source files
      npsrc = 0
      j=1
      k=1
      do i=1,nsrc*maxdim,maxdim
        ! store source locations in temp array
        tpsrc(j,1) = locs(i)
        tpsrc(j,2) = locs(i+1)
        tpsrc(j,3) = locs(i+2)
        tpsrc(j,3) = nz + 1 - tpsrc(j,3)

        ! compute offset in proc space
        nxp = int((tpsrc(j,1) - 1) / nxt)
        nyp = int((tpsrc(j,2) - 1) / nyt)
        nzp = int((tpsrc(j,3) - 1) / nzt)

        ! check if source locs are in bounds
        if (nxp .lt. 0 .or. nxp .gt. dims(1)) then
            print*, 'fault x-component node location(s) incorrect'
            call MPI_Abort()
            call exit(-1)
        endif
        if (nyp .lt. 0 .or. nyp .gt. dims(2)) then
            print*, 'fault y-component node location(s) incorrect'
            call MPI_Abort()
            call exit(-1)
        endif
        if (nzp .lt. 0 .or. nzp .gt. dims(2)) then
            print*, 'fault z-component node location(s) incorrect'
            call MPI_Abort()
            call exit(-1)
        endif

        ! determine if source belongs to node
        if (coords(1).eq.nxp .and. coords(2).eq.nyp .and. coords(3).eq.nzp) then
            ! update to local coords in proc space
            if (mod(tpsrc(j,1),nxt) /= 0) then
                tpsrc(j,1) = mod(tpsrc(j,1),nxt)
            else
                tpsrc(j,1) = nxt
            endif
            if (mod(tpsrc(j,2),nyt) /= 0) then
                tpsrc(j,2) = mod(tpsrc(j,2),nyt)
            else
                tpsrc(j,2) = nyt
            endif
            if (mod(tpsrc(j,3),nzt) /= 0) then
                tpsrc(j,3) = mod(tpsrc(j,3),nzt)
            else
                tpsrc(j,3) = nzt
            endif
            npsrc=npsrc+1
      ...
      ! copy over to permanent arrays
          do i=1,npsrc
            psrc(i,1) = tpsrc(i,1)
            psrc(i,2) = tpsrc(i,2)
            psrc(i,3) = tpsrc(i,3)
          end do
      print*, 'npsrc: ', npsrc, ' sources assigned to rank: ', rank
      endif
      ...
      subroutine read_source_ifault_5(npsrc, read_step, nst, comm, idx, insrc)
      use parstat
      implicit none
     
      integer                                                   :: npsrc, comm
      integer                                                   :: nst, i, stride
      integer                                                   :: read_step
      ...
      integer(kind=MPI_ADDRESS_KIND), dimension(:), allocatable :: disps
      integer(kind=MPI_ADDRESS_KIND)                            :: goff,loff
      character(len=180)                                        :: insrc, errstr
      integer                                                   :: fh,errlen
      integer                                                   :: idx ! number of times called 
      integer                                                   :: ix,iy
      ...
      if (npsrc.ne.0) then
          ...
          ! allocate stuff
          allocate(buffer(6*read_step*npsrc))
          allocate(disps(npsrc))
          allocate(block_lengths(npsrc))

          stride=3+6*nst
          disp=0
          do i=1,npsrc
              ! note: not platform independent. assuming 64-bit, 4-byte real.
              ! global offset
              goff = 4*(3+int8(stride)*int8(src_locs(i)-1))
              ! local offset
              loff = 4*(6*int8(read_step)*(int8(idx)-1))
              disps(i)=goff+loff
              block_lengths(i)=read_step*6
          enddo

          ! mpi datatype
          call MPI_Type_create_hindexed(npsrc,block_lengths,disps,MPI_REAL,type1,ierr)
          call MPI_Type_commit(type1, ierr)

          ! file i/o stuff
          call MPI_File_open(comm, insrc, MPI_MODE_RDONLY, MPI_INFO_NULL, fh, ierr)
          call MPI_File_set_view(fh, disp, MPI_REAL, type1, "native", MPI_INFO_NULL, ierr)
          call MPI_File_read_all(fh, buffer, npsrc*read_step*6, MPI_REAL, MPI_STATUS_IGNORE, ierr)
          call MPI_File_close(fh, ierr)

          ! copy over
          do ix=1,npsrc
            do iy=1,read_step
               i=(ix-1)*read_step*6+(iy-1)*6
               axx(ix,iy) = buffer(i+1)
               ayy(ix,iy) = buffer(i+2)
               azz(ix,iy) = buffer(i+3)
               axz(ix,iy) = buffer(i+4)
               ayz(ix,iy) = buffer(i+5)
               axy(ix,iy) = buffer(i+6)
            enddo
          enddo

          ! clean up
          deallocate(disps)
          deallocate(block_lengths)
          deallocate(buffer)
      endif
...
C.2     update coords to local proc coords
        j=1
        do i=1,npsrc
            nxp = int((tpsrc(i,1)-1)/nxt)
            nyp = int((tpsrc(i,2)-1)/nyt)
...
C       MARCH OVER NUMBER OF FAULT NODES  IN PROC
      if (ifault==2 .or. ifault==5) then
        ...
        !print*, axx(1,k),ayy(1,k),azz(1,k),axz(1,k),ayz(1,k),axy(1,k)
