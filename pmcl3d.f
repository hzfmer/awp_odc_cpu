...
c     INTEGRATION/PERFORMANCE/EXTRA VARIABLES    
      ...
      integer :: MCW,MC1,MFS,MCR,MCR2,iocomm
...
C     EXTREME MEDIA VALUES AND Q BANDWIDTH DESCRIPTION
      ...
      real :: fac,Q0,ex,fp,maxvpe,minvpe,maxvse,minvse,maxdde,mindde
...
C      MATERIAL PARAMETERS FOR HOMOGENOUS MESH OPTION
      real :: vp_in, vs_in, rho_in
...
C       call openlog(1)
     ...
     +       mu_ss,mu_dd,fac,
     +       Q0,ex,fp,chkp,chkj,insrc,invel,sxgro,sygro,szgro,
     +       sxgro2,sygro2,szgro2,read_step,write_step,write_step2,sgsn,
     +       filen,pht,
     +       ifault,checkpoint,isfcvlm,imd5,ivelocity,
     +       mediarestart,vp_in,vs_in,rho_in,io_opt,iperf,idyna,nvar,iost,partdeg,SoCalQ)
...
C     ==========================
      ...
      call MPI_BCAST(vp_in,1,MPI_REAL,master,MCW,err)
      call MPI_BCAST(vs_in,1,MPI_REAL,master,MCW,err)
      call MPI_BCAST(rho_in,1,MPI_REAL,master,MCW,err)
      if(rank==master) print*, 'vp vs rho', vp_in, vs_in, rho_in
...
c     ----------------------------------------------------------
      ...
      call MPI_BCAST(fac,    1,MPI_REAL,    master,MCW,err)
      call MPI_BCAST(Q0,    1,MPI_REAL,    master,MCW,err)
      call MPI_BCAST(ex,    1,MPI_REAL,    master,MCW,err)
...
C     ==============================================
      call inisource(rank,size1,MC1,master,insrc,dims,coords,maxdim,
     ...
     +              ifault,idyna,iocomm)
      if (ifault.eq.5) then
          ! create io-comm
          if (rank==srcproc) color2=1
          call mpi_comm_split(MC1,color2,0,iocomm,err)
          ! read source with idx = 1
          call read_source_ifault_5(npsrc, read_step, nst, iocomm, 1, insrc)
      endif
...
c       taumin=1./(400.)*.1*fac
cKBO 8/30/12
c      taumax=1./(0.01*2.*3.1415927)*10.0*fac !K.W. original frequncy range is 0.01 to 400 

      if ((ex .LT. 0.65) .AND. (ex .GT. 0.01)) then
c      taumin=1./(400.*2.*3.1415927)*.1*fac    
...
cKBO 8/30/12
     ...
     +            taumin,taumax,fac,Q0,ex,fp,bsize,y_dt,idyna,nve,mediarestart,nvar,iost,partdeg,SoCalQ,vp_in,vs_in,rho_in)
...
Ckw      if(nve==1) call tausub(taumin,taumax) 
Ckw      if(nve==1) call weights_sub(ex)
C     ===================================================
...
C     ===================================================
     ...
     +       nbgz,nedz,nskpz,vpe,vse,dde,nve,fac,Q0,ex,fp)
...
c     if(fsf==1) call fstr(nxt,nyt,nzt)
     ...
     +                 toe,fromw,tow,frome,tou,fromd,tod,fromu)   
               ...
               ! this needs cleaned up into a single call
     ...
     +                 nsrc,nst,npsrc,nz,i/read_step +1,read_step)
               ...
               if (ifault==5 .and. nst.gt.read_step .and. mod(i,read_step)==0 .and. i.lt.nst)  then
                  call read_source_ifault_5(npsrc, read_step, nst, iocomm, i/read_step+1, insrc)   
               end if
            ...
            if(rank==recproc) then
               ...
               if(i==it) then
                  if (count == 0) then
                     ...
                     if (ifault .le. 2 .or. ifault .eq. 5) then
...
c     end if
     ...
     +                          szgro_md5,imd5,ifault,1,filen)
                  ...
                  if ((count == write_step) .or. (it==(ntiskp*int(nt/ntiskp)))) then
                     deallocate(bufx)
                     deallocate(bufy)
                     if ((ifault .le. 2) .or. (it==(ntiskp*int(nt/ntiskp))).or. (ifault.eq.5)) then
                        deallocate(bufz)
                     end if
